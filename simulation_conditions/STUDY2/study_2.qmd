...
**Implementation in this report.** All bias and coverage summaries for the copula parameter use $\rho_{\mathrm{eff}}$ as the target in mixed-direction cells. The facet labels still display the *input* $\rho\in\{0.30,0.50\}$ so that the grid remains comparable to Study 1.
...
```{r exp_bias, fig.height=12, fig.width=16}
#| label: exp_bias
#| echo: false
...
plot_metric(
  ...
  title = "Relative bias (Study 2: Exponential DGP)",
  use_free_y = TRUE
)
```
...
```{r exp_coverage, fig.height=12, fig.width=16}
#| label: exp_coverage
#| echo: false
...
plot_metric(
  ...
  title = "95% interval coverage (Study 2: Exponential DGP)",
  ylims = c(0.5, 1.0)
)
```
...
```{r exp_sdbias, fig.height=12, fig.width=16}
#| label: exp_sdbias
#| echo: false
...
plot_metric(
  cond_core,
  metric_col = "sd_bias",
  ylab = "SD-bias (mean posterior SD − empirical SD)",
  title = "SD-bias (Study 2: Exponential DGP)",
  use_free_y = TRUE
)
```
...
```{r marginal_params, fig.height=8, fig.width=16}
...
```{r marginal_params_ng, fig.height=8, fig.width=16}
...
cond_status <- rep_df |>
  filter(param %in% core_params, Model == "Exponential–Gaussian (EG)", mcmc_status != "Failed/Error") |>
  group_by(T, skew_level, direction, VARset, rho, param, mcmc_status) |>
  summarise(coverage_95 = mean(cover95_eff, na.rm = TRUE), .groups = "drop")

if (nrow(cond_status) > 0 && length(unique(cond_status$mcmc_status)) > 1) {
  ggplot(cond_status, aes(x = T, y = coverage_95, color = mcmc_status, group = mcmc_status)) +
    ...
    theme_standard +
    labs(
      title = "EG coverage by MCMC status (Clean vs Problematic)",
