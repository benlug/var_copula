...
| `adapt_delta` | 0.995 | Target acceptance probability |
| `max_treedepth` | 15 | Maximum tree depth for NUTS |
| Parallelization | Outer loop | Replications parallelized; chains run sequentially |

::: {.callout-note}
## Higher adapt_delta Than Study 1

Study 2 uses `adapt_delta = 0.995` compared to 0.95 in Study 1. This higher value was chosen because the EG model's feasibility-bound reparameterization creates regions where the posterior density changes rapidly near the boundary. The elevated target acceptance rate reduces step sizes in these regions, preventing divergent transitions.

Despite this challenging geometry, **zero divergent transitions** were observed across all 14,400 fits (7,200 per model), demonstrating that the reparameterization strategy (Section 6.4) successfully regularizes the posterior.
:::
...
### The Support Constraint Problem
...
### Data-Dependent Feasibility Bound
...
1. **Automatic feasibility**: $\sigma_{\mathrm{exp}} > b$ by construction for any $\eta \in \mathbb{R}$
2. **Smooth geometry**: No hard boundaries for the sampler to navigate
3. **Well-defined gradients**: The transformation is smooth and invertible
...
## 6.6. Reproducibility Strategy

The simulation study employs deterministic seeding to ensure full reproducibility.

### Data Generation Seeds

Each simulated dataset receives a unique seed:

$$
\text{seed}_{\text{data}} = \text{seed\_base} + \text{condition\_id} \times 100000 + \text{rep\_id}
$$

with $\text{seed\_base} = 2026$ for Study 2. The larger multiplier (100,000 vs 10,000 in Study 1) provides wider separation between condition seed ranges.

### MCMC Initialization Seeds

The EG model uses data-driven initialization based on OLS pre-fits:

1. Fit OLS: $y_{1,t} \sim y_{1,t-1} + y_{2,t-1}$ and $y_{2,t} \sim y_{1,t-1} + y_{2,t-1}$
2. Extract coefficients as initial values for $\mu$ and $\Phi$
3. Compute feasibility bounds $b$ from OLS residuals
4. Set $\eta = \log(0.2)$ (slack ≈ 0.2) as default

For NG, standard random initialization is used with seed:

$$
\text{seed}_{\text{init}} = \text{seed\_base}_R + \text{seed\_key}(\text{cid}, \text{rid}, \text{chain})
$$

where $\text{seed\_key}(\text{cid}, \text{rid}, \text{chain}) = \text{cid} \times 100000 + \text{rid} \times 10 + \text{chain}$.

### Stan Sampler Seeds

$$
\text{seed}_{\text{Stan}} = \text{seed\_base}_{\text{Stan}} + \text{seed\_key}(\text{cid}, \text{rid}, 0)
$$

with $\text{seed\_base}_{\text{Stan}} = 5 \times 10^6$.

::: {.callout-note}
## Resume Capability

The pipeline supports resumption via environment variables `START_COND` and `START_REP`. Because seeds are deterministic functions of (condition, replication), resuming from any point produces identical results to running the full pipeline.
:::

### Initialization Sidecar Files

For each fitted model, the pipeline optionally saves a "sidecar" file containing:

- All random seeds used (data, init, Stan)
- Exact initial values for each chain
- Timestamp and metadata

This enables exact reproduction of any individual fit.
