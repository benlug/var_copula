...
The Normal–Gaussian (NG) model is computationally stable across all simulation conditions (no post-warmup divergent transitions; max $\widehat{R} \le 1.01$ in all replications). The Skew–Gaussian (SG) model becomes progressively harder to sample as marginal skewness increases: the proportion of replications classified as *Problematic* (max $\widehat{R} > 1.01$ and/or at least one divergent transition) rises sharply and is essentially universal under `extremeCHI`.

Within this design, these coarse diagnostics are weak predictors of frequentist operating characteristics for the VAR dynamics. In particular, SG coverage for $\Phi$ is similar in *Clean* and *Problematic* subsets. This empirical robustness does not extend to the copula correlation $\rho$, which is substantially attenuated under marginal misspecification for both models.

## 0.2 Model Performance Across Skewness Conditions

**Moderate skewness** (`moderateSN`, $\alpha = \pm 4$): NG and SG exhibit broadly comparable performance. For $T \ge 100$, bias is near zero and coverage is close to 0.95 for all core parameters. At $T = 50$, both models show mild finite-sample attenuation in some $\Phi$ and $\rho$ components, but the magnitudes are small and do not materially affect coverage. Under this level of skewness, SG’s additional marginal flexibility yields limited inferential benefit.

**Strong skewness** (`strongSN`, $\alpha = \pm 9$): Differences between NG and SG emerge for the VAR dynamics. SG typically reduces relative bias in $\Phi$ by approximately 0.05–0.15 (occasionally larger at $T = 50$), while coverage gains remain modest. Both models exhibit appreciable downward bias in $\rho$. Because the DGP uses a Gaussian copula, this pattern is best interpreted as PIT distortion induced by marginal misspecification (and finite-sample effects), rather than as an intrinsic failure of Gaussian copulas.
