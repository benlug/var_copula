...
### 0.1 Computational vs. inferential

- **NG** is computationally robust across conditions (nearly all runs “Clean”); **SG** accumulates problematic diagnostics as skewness increases (especially at \(T=50\) under `strongSN` and in nearly all runs for `extremeCHI`). This is visible in the status counts and in the divergence boxplots.  
- **Divergences != automatic invalidity.** For **SG**, coverage of the VAR dynamics $\Phi$ looks similar between “Clean” and “Problematic” runs; however, $\rho$ remains poor regardless of status. Model misspecification dominates $\rho$.

### 0.2 Where NG is adequate and where it breaks

- **`moderateSN` (\(\alpha=\pm 4\))**: **NG \(\approx\) SG in practice.** Biases are tiny on the displayed scales (e.g., \(\mu\) panels \(\pm 0.02\)) and coverage hovers near \(0.95\) for both models. No practical advantage for SG at this skewness level.

- **`strongSN` (\(\alpha=\pm 9\))**: **SG improves \(\Phi\) modestly** (relative‑bias differences typically within \(\pm 0.1\text{–}0.3\); coverage gains are **hundredths** on a \(0.80\text{–}1.00\) axis). **\(\rho\)** is still down‑biased for both models. **SD‑Bias** magnitudes are **small** (often \(\pm 0.02\text{–}0.03\)), so “massive overconfidence” is not warranted here.

- **`extremeCHI` (\(\chi^2_1\) standardized)**: **Both models are misspecified.**
  - **\(\Phi\):** **SG** retains **small relative bias** (rows typically \(-0.3\ldots 0.1\)) and **coverage \(\sim 0.90\text{–}0.97\)** (worse at \(T=50\)), whereas **NG** is clearly worse.  
  - **\(\rho\):** **Severe failure for both**—relative bias often \(< -1\) on a \(\rho\) RB axis spanning \(-2\ldots 0\); this implies **frequent sign flips**, not just shrinkage toward zero. Coverage can be well below \(0.9\) on a \(0.5\text{–}1.0\) axis.  
  - **\(\mu\):** **SG shows visible bias** (\(\mu\) axes \(-0.2\ldots 0.2\)), while **NG** is near zero. **SD‑Bias** is parameter‑specific: **\(\mu\)** is **under‑dispersed** (as low as \(-0.1\) on a \(-0.10\ldots 0\) axis), **\(\Phi\)** is **slightly conservative** (about \(0\) to \(+0.03\)), and **\(\rho\)** is **under‑dispersed** (about \(-0.03\) to \(-0.05\)). These patterns align with **marginal misfit** and **PIT distortion** as the driving mechanism.

### 0.3 Mechanism (reframed)

- **Primary driver: PIT distortion from marginal misfit**, especially under `extremeCHI`. Changes in **\(\sigma\)** for NG are **small** (\(\approx \pm 0.025\)) and cannot explain the large \(\Phi/\rho\) distortions. The key phenomenon is **non‑uniform PIT** when the assumed marginals (Normal / Skew‑Normal) cannot reproduce true skewness and tails, leading to **attenuated—and often sign‑reversed—\(\rho\)**. Under severe misfit, **location \((\mu)\)** in SG absorbs shape error, producing the observed \(\mu\) bias.
...
### 1.3 Visual check: standardized marginal innovations (DGP)
...
#### Interpetation: High Bias for SN Model in intercepts ($\mu_1$, $\mu_2$)
...
**Interpretation:** The \(\sigma\) bias under NG is **small** (about \(\pm 0.025\) on a tight y‑axis; see p. 23), so it **cannot** by itself explain the large distortions in \(\rho\) and (to a lesser extent) \(\Phi\) under `extremeCHI`. It is better viewed as a **by‑product** of marginal misspecification (some skewness/heavy tails leaking into scale), whereas the **dominant mechanism** behind \(\rho\) failure is the **PIT distortion** described above. :contentReference[oaicite:6]{index=6}
...
2. **Identification improves with \(T\).** The \(\alpha\) panels (p. 24) show bias shrinking as \(T\) grows, consistent with increasing information about asymmetry. This is mirrored by improving \(\Phi\) coverage for SG as \(T\) increases in the SN DGPs (pp. 15–16). :contentReference[oaicite:7]{index=7}
