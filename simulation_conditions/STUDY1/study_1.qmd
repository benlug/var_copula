...
The Normal-Gaussian (NG, Normal Margins, Gaussian Copula) model demonstrates robust computational performance across all simulation conditions, achieving convergence without divergent transitions in all replications. In contrast, the Skew-Gaussian (SG, Skew-Normal Margins, Gaussian Copula) model exhibits increasing computational instability as marginal skewness increases, with divergence rates approaching 100% under extreme misspecification. 
...
**Interpretation:** The $\sigma$ bias under NG is small (about $\pm 0.025$ on a tight y‑axis), so it cannot by itself explain the large distortions in $\rho$ and (to a lesser extent) $\Phi$ under `extremeCHI`. It is better viewed as a y‑product of marginal misspecification (some skewness/heavy tails leaking into scale), whereas the dominant mechanism behind \(\rho\) failure is the PIT distortion described above.
