...
// Mirrors parameter names to align with analysis code.

functions {
  real exp_margin_lpdf(real e, real s, int dir) {
    if (dir == 1) { real x = e / s + 1; if (x < 0) return negative_infinity();
      return exponential_lpdf(x | 1) - log(s);
    } else { real x = 1 - e / s; if (x < 0) return negative_infinity();
      return exponential_lpdf(x | 1) - log(s);
    }
  }
  ...
  real gauss_copula_lpdf(real u1, real u2, real rho) {
    ...
    z[1]=inv_Phi(fmin(fmax(u1,1e-9),1-1e-9)); z[2]=inv_Phi(fmin(fmax(u2,1e-9),1-1e-9));
...
model {
  ...
  // State transition with Exponential innovations ζ
  for (j in 1:2) state[1,j] ~ normal(0, 1);
  for (t in 2:T) {
    vector[2] mean_t = mu + B * to_vector(state[t-1]');
    // ζ_t = state[t] - mean_t
    real z1 = state[t,1] - mean_t[1];
    real z2 = state[t,2] - mean_t[2];
  ...
  // Measurement equation: y_t = state_t (Θ = 0)
  // (No additional likelihood; deterministic mapping used in GQ if needed.)
}

generated quantities {
  // none required beyond parameters.
}
