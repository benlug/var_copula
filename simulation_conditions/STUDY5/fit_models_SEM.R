...
# • Defaults: adapt_delta=0.90, max_treedepth=10
# • EI (indicator-skew) inits: build initial state path (z_raw=0),
#   then set sigma_exp from e = y - state_init  → feasible at init
# • EL (latent-skew) inits: data-driven B; sigma_exp support-aware
# • Safe logging: ensure directory exists; touch file; fall back to console
# • Cached compilation keyed by Stan source hash
###########################################################################
...
fit_sem_models <- function(data_dir, fits_dir, stan_dir, results_dir,
  ...
  suppressPackageStartupMessages({
    ...
    library(rstan); library(stringr); library(digest)
    library(doParallel); library(foreach); library(dplyr)
  })
  ...
  # Ensure parent exists and the file is creatable (touch)
  dir.create(dirname(log_file), showWarnings = FALSE, recursive = TRUE)
  if (!file.exists(log_file)) {
    try(file.create(log_file), silent = TRUE)
  }
  ...
  model_cache <- function(path, tag) {
    ...
    old <- setdiff(old, rds); if (length(old)) try(unlink(old), silent = TRUE)
    if (file.exists(rds)) { message("Loading cached: ", tag, " [", substr(h,1,8), "]"); readRDS(rds)
    } else { message("Compiling: ", tag, " [", substr(h,1,8), "]")
  ...
  # --- helpers -----------------------------------------------------------
  .min_sigma_for_dir <- function(r, dir) {
    saf <- 1.05
    if (dir == 1L) max(1, saf * max(-r, na.rm = TRUE)) else max(1, saf * max(r, na.rm = TRUE))
  }

  # EI: build initial state path (z_raw=0), then set sigma_exp from e = y - state_init
  make_init_EI <- function(y, skew_dir, chain_id) {
    set.seed(SEED_BASE_R + chain_id)
    T_len <- nrow(y)

    mu0   <- rnorm(2, 0, 0.05)
    phi11 <- 0.55; phi12 <- 0.10
    phi21 <- 0.10; phi22 <- 0.25
    B     <- matrix(c(phi11, phi12, phi21, phi22), 2, 2, byrow = TRUE)

    state <- matrix(0, T_len, 2)
    s <- mu0
    state[1, ] <- s
    if (T_len >= 2) for (t in 2:T_len) { s <- mu0 + B %*% s; state[t,] <- s }

    e  <- y - state
    s1 <- .min_sigma_for_dir(e[,1], skew_dir[1])
    s2 <- .min_sigma_for_dir(e[,2], skew_dir[2])
    ...
    list(
      ...
      eta   = log(c(s1, s2)),
      rho   = runif(1, -0.2, 0.2),
      z_raw = matrix(0, T_len, 2)
    )
  }

  # EL: data-driven B; sigma_exp chosen to satisfy support of initial zeta residuals
  make_init_EL <- function(y, skew_dir, chain_id) {
    ...
    z  <- cbind(
      y[,1] - (mu0[1] + phi11*ylag[,1] + phi12*ylag[,2]),
      y[,2] - (mu0[2] + phi21*ylag[,1] + phi22*ylag[,2])
    )[-1, , drop = FALSE]

    s1 <- .min_sigma_for_dir(z[,1], skew_dir[1])
    s2 <- .min_sigma_for_dir(z[,2], skew_dir[2])
  ...
  if (!length(paths)) { message("No data files found in ", data_dir); return(invisible()) }

  keep <- (as.integer(meta[,2]) > start_condition) |
          (as.integer(meta[,2]) == start_condition & as.integer(meta[,3]) >= start_rep)
  paths <- paths[keep]; if (!length(paths)) { message("Nothing to fit in requested range."); return(invisible()) }
  ...
  log_vec <- foreach::foreach(
    ...
    cid <- as.integer(mm[2]); rid <- as.integer(mm[3])

    ds <- try(readRDS(p), silent = TRUE)
    if (inherits(ds, "try-error")) return(sprintf("ERROR cond%03d rep%03d: readRDS failed", cid, rid))
    ...
    for (code in c("EI","EL")) {
      ...
      if (file.exists(fpath)) {
        ...
        if (inherits(chk,"stanfit") && length(chk@sim) > 0 && length(chk@sim$samples) > 0) {
          ...
          next
        } else try(unlink(fpath), silent = TRUE)
      }
      ...
      try(saveRDS(list(meta=list(model=code, cid=cid, rid=rid, time=Sys.time()),
      ...
      if (inherits(fit, "stanfit") && length(fit@sim) > 0) {
        ...
        msgs <- c(msgs, sprintf("%s cond%03d rep%03d : OK (%s div)",
  ...
  # --- safe log write ----------------------------------------------------
  if (length(log_vec)) {
    # Re-check existence (in case the dir was deleted mid-run)
    dir.create(dirname(log_file), showWarnings = FALSE, recursive = TRUE)
    ok_conn <- FALSE
    ...
    if (!inherits(con, "try-error")) {
      ...
      # Fall back to console if we still cannot open the file
      message("Logger fallback (couldn't open ", log_file, "):")
