...
for (i in seq_along(fits)) {
  fp <- fits[i]
  m  <- stringr::str_match(basename(fp), "fit_(EI|EL)_cond(\\d+)_rep(\\d+)\\.rds")
  model_code <- m[2]; cid <- as.integer(m[3]); rid <- as.integer(m[4])
  ...
  if (inherits(sim, "error")) {
    rows[[i]] <- tibble(model = model_code, condition_id = cid, rep_id = rid,
  ...
  rows[[i]] <- if (is.null(sm)) {
    tibble(model = model_code, condition_id = cid, rep_id = rid,
