...
# STUDY 3 ADDITION: Gamma parameters (SD of standardized Gamma residuals)
EXTRA_GG <- c("sigma_gam[1]", "sigma_gam[2]")

# ── enumerate fits -------------------------------------------------------
# Update regex to include EG
fits <- list.files(FITS_DIR, "^fit_(SG|NG|EG|GG)_cond\\d+_rep\\d+\\.rds$",
  full.names = TRUE
)
...
for (i in seq_along(fits)) {
  fp <- fits[i]
  # Update regex to include EG / GG
  m <- str_match(basename(fp), "fit_(SG|NG|EG|GG)_cond(\\d+)_rep(\\d+)\\.rds")
  model <- m[2]
  cid <- as.integer(m[3])
  rid <- as.integer(m[4])
  ...
  # STUDY 2/3: Positive-support marginal Ground Truth
  # If the DGP is Exponential or Gamma, the standardized residual SD is 1.
  is_exponential_dgp <- (!is.null(sim$true_params$margin1$type) && sim$true_params$margin1$type == "exponential")
  is_gamma_dgp <- (!is.null(sim$true_params$margin1$type) && sim$true_params$margin1$type == "gamma")
  true_sigma_exp <- if (is_exponential_dgp) 1 else NA_real_
  true_sigma_gam <- if (is_gamma_dgp) 1 else NA_real_
  ...
  if (model == "GG") {
    # STUDY 3 ADDITION
    truth <- c(truth, `sigma_gam[1]` = true_sigma_gam, `sigma_gam[2]` = true_sigma_gam)
  }
