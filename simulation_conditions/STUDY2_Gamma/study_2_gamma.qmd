---
title: "Study 2: Comparative Performance of Normal–Gaussian and Gamma–Gaussian Copula VAR Models Under Gamma Innovations (Three Skewness Levels)"
format:
  pdf:
    toc: true
    toc_depth: 3
...
with $\mu = \mathbf{0}$. Innovations $\varepsilon_t = (\varepsilon_{1t}, \varepsilon_{2t})$ have standardized Gamma margins (mean 0, variance 1), with optional mirroring to induce left-skewness, and are coupled through a Gaussian copula with correlation parameter $\rho$.

::: {.callout-note}
## Standardization of Gamma Innovations

A Gamma random variable $X \sim \text{Gamma}(k, \beta)$ (shape $k>0$, rate $\beta>0$) has

$$
\mathbb{E}[X] = \frac{k}{\beta}, \qquad \text{Var}(X) = \frac{k}{\beta^2}.
$$

To obtain mean 0 and variance 1 innovations, we apply the standardization

$$
Z = \frac{X - k/\beta}{\sqrt{k}/\beta}.
$$

In the simulation pipeline we use $\beta = \sqrt{k}$ so that $\text{Var}(X)=1$ and $Z = X - \sqrt{k}$. The (right) skewness magnitude is $2/\sqrt{k}$, so larger $k$ implies less skew. Mirroring ($-Z$) produces left-skewed innovations. For the joint density via Gaussian copula, see Study 1, Section 1.1.
:::

<!-- ::: {.callout-important} -->
<!-- ## Copula Sign Under Mirroring -->

<!-- Left-skew Gamma margins are produced by mirroring standardized Gamma innovations (multiplying by $-1$). For a Gaussian copula, mirroring exactly one margin corresponds to the transformation $u \mapsto 1-u$ on the PIT scale. Since $\Phi^{-1}(1-u) = -\Phi^{-1}(u)$, this flips the sign of the latent Gaussian score and therefore maps -->
...
The study employs a factorial design crossing five factors.
...
design_summary <- tibble(
  ...
  Levels = c(
    "Standardized Gamma",
    "$k \u2208 \\{2.0, 1.0, 0.5\\}$ (least \u2192 most skew; skewness $=2/\\sqrt{k}$)",
    "50, 100, 200",
...
true_params <- tibble(
  Parameter = c(
    ...
    "$\\sigma_{\\mathrm{gam},1}, \\sigma_{\\mathrm{gam},2}$ (GG model)",
    "$k$ (GG Gamma shape)"
  ),
  `True Value` = c(
    "0, 0",
    ...
    "1.0, 1.0",
    "1.0, 1.0",
    "Varies by condition: 2.0 / 1.0 / 0.5 (set via `GAMMA_SHAPES`)"
  ),
  Notes = c(
    ...
    "Scale parameter for standardized Gamma",
    "Gamma shape parameter (skewness = $2/\\sqrt{k}$)"
  )
)
...
# Three skewness levels via Gamma shape k (larger k = less skew).
# The "current" setting in the pipeline (k=2) is the least-skewed level.
shape_levels <- c(2, 1, 0.5)

plot_df <- purrr::map_dfr(shape_levels, function(k) {
  u <- runif(N_draw)
  rate_k <- sqrt(k)
  # With rate = sqrt(k): Var(X)=1, mean(X)=sqrt(k)
  x_raw <- qgamma(u, shape = k, rate = rate_k)
  x_std <- x_raw - sqrt(k)
  tibble(
    value = c(x_std, -x_std),
    dist  = rep(c("Right-skew (std.)", "Left-skew (mirrored)"), each = N_draw),
    k_lab = paste0("k = ", k, "  (skewness ", round(2 / sqrt(k), 2), ")")
  )
})
...
ggplot(plot_df, aes(x = value, fill = dist)) +
  ...
  facet_grid(k_lab ~ dist, scales = "free") +
  theme_bw(base_size = 12) +
  labs(
    title = "Standardized Gamma innovations used in Study 2 (Gamma extension)",
    x = "value",
    y = "density"
  ) +
  guides(fill = "none")
...
if (!"skew_level" %in% names(design_raw)) {
  if ("dgp_level" %in% names(design_raw)) {
    ...
    # if the design grid has no DGP label column, treat it as fixed Gamma
    design_raw <- design_raw |> mutate(skew_level = "Gamma")
  }
}
...
# Order skew_level from least to most skew if shape information is available
# (larger k = less skew).
if ("shape_k" %in% names(design_raw)) {
  skew_levels_ordered <- design_raw |>
    distinct(skew_level, shape_k) |>
    arrange(desc(shape_k)) |>
    pull(skew_level)
} else {
  skew_levels_ordered <- sort(unique(design$skew_level))
}
...
param_levels <- c(
  # GG marginal parameters
  "sigma_gam[1]", "sigma_gam[2]", "shape_gam",
  # NG marginal sds
  "sigma[1]", "sigma[2]",
  # core parameters
...
prep_data <- function(df) {
  df |>
    mutate(
      ...
      T = factor(T),
      skew_level = factor(skew_level, levels = skew_levels_ordered),
      ...
      Model = factor(
        ifelse(model == "GG", "Gamma–Gaussian (GG)", "Normal–Gaussian (NG)"),
        levels = c("Normal–Gaussian (NG)", "Gamma–Gaussian (GG)")
      )
    )
}
...
Both NG and GG models exhibit uniformly clean fits across the entire design, with almost no post-warmup divergent transitions. This favorable computational performance contrasts with the SG model under extremeCHI in Study 1, which experienced frequent divergences. The absence of sampling difficulties in the current analysis indicates that correct specification of the marginal distribution, as in GG, mitigates the posterior geometry challenges associated with model misspecification.
...
# 4. Gamma DGP: NG vs GG Performance
...
**Interpretation.** Across $T$, GG is approximately unbiased for $\Phi$ and $\mu$. For $\rho$, the target in mixed-direction cells is $\rho_{\mathrm{eff}}$ (callout above): GG is close to unbiased, while NG exhibits attenuation in $|\rho|$, most pronounced at small $T$.
...
**Interpretation.** GG coverage is typically close to nominal for the core parameters. NG coverage shortfalls are concentrated in $\rho$, consistent with attenuation bias under marginal misspecification.
...
**Interpretation.** GG is generally well calibrated (SD-bias near 0). NG frequently shows negative SD-bias for $\rho$, indicating overconfident posterior uncertainty for dependence under misspecified margins.

# 5. Marginal Parameters

We report mean bias for the Gamma scale parameters in GG ($\\sigma_{\\mathrm{gam}}$) and the innovation standard deviations in NG ($\\sigma$). We additionally report bias for the Gamma shape parameter $k$ (denoted `shape_gam`). Under the standardized DGP, the reference value for $\\sigma_{\\mathrm{gam}}$ and $\\sigma$ is 1.
...
# GG marginal scales
sigma_gam_data <- cond_all |>
  filter(param %in% c("sigma_gam[1]", "sigma_gam[2]"), Model == "Gamma–Gaussian (GG)")

if (nrow(sigma_gam_data) > 0) {
  plot_metric_readable(
    sigma_gam_data,
    metric_col = "mean_bias",
    ylab = "Mean bias (estimate − 1)",
    title = "GG: bias for sigma_gam (truth = 1)",
    params = c("sigma_gam[1]", "sigma_gam[2]"),
    use_free_y = TRUE
  )
} else {
  message("No GG marginal scale parameters available.")
}

# GG shape parameter (if estimated)
shape_gam_data <- cond_all |>
  filter(param %in% c("shape_gam"), Model == "Gamma–Gaussian (GG)")

if (nrow(shape_gam_data) > 0) {
  plot_metric_readable(
    shape_gam_data,
    metric_col = "mean_bias",
    ylab = "Mean bias (estimate − truth)",
    title = "GG: bias for shape_gam (truth set by DGP)",
    params = c("shape_gam"),
    use_free_y = TRUE
  )
} else {
  message("No GG shape parameter available (shape may be fixed in Stan).")
}
```
...
This section provides technical details on the implementation of the simulation study, including prior specifications, MCMC settings, and mathematical derivations specific to the Gamma–Gaussian (GG) model.

## 6.1. Prior Specifications

Both models use weakly informative priors. The GG model employs a feasibility-bound reparameterization for the marginal scale parameters to handle the positive support constraint implied by Gamma marginals.
...
**Table: Prior Specifications for the Gamma–Gaussian (GG) Model**

| Parameter | Prior | Support | Rationale |
|-----------|-------|---------|-----------|
| $\mu_1, \mu_2$ | $\text{Normal}(0, 1)$ | $\mathbb{R}$ | Weakly informative; centered at true value (0) |
| $\phi_{11}, \phi_{12}, \phi_{21}, \phi_{22}$ | $\text{Normal}(0, 0.5)$ | $(-1, 1)$ | Regularizes toward stationarity; truncated by bounds |
| $\sigma_{\mathrm{gam},1}, \sigma_{\mathrm{gam},2}$ | $\text{Log-Normal}(0, 0.5)$ | $(b_i, \infty)$ | Induced prior via reparameterization (see Section 6.4) |
| $k$ (`shape_gam`) | $\text{Log-Normal}(\log 2, 0.5)$ | $(0, \infty)$ | Weakly informative; centers near the default DGP shape |
| $\rho$ | $\text{Normal}(0, 0.5)$ | $(-1, 1)$ | Regularizes toward independence; truncated by bounds |

::: {.callout-note}
## GG Scale Parameter Prior

The GG model does **not** place a direct prior on the unconstrained auxiliary parameters $\eta \in \mathbb{R}$. Instead, it estimates $\eta$ and applies the change-of-variables transformation:

$$
\sigma_{\mathrm{gam}} = b + \exp(\eta)
$$

where $b$ is a data- (and shape-) dependent feasibility bound (Section 6.4). The induced prior on $\sigma_{\mathrm{gam}}$ is $\text{Log-Normal}(0, 0.5)$ with support shifted to $(b, \infty)$.
:::
...
## 6.4. GG Model: Feasibility-Bound Reparameterization

The Gamma–Gaussian model requires that the shifted residuals have positive support. This section documents the reparameterization strategy that ensures feasibility while maintaining good posterior geometry.

For a Gamma marginal with scale $\sigma_{\mathrm{gam}}$, the likelihood requires:

$$
x_{\text{shifted},t} = \sqrt{k}\,\sigma_{\mathrm{gam}} + s \cdot \text{res}_t > 0, \quad \forall\, t
$$

where $s \in \{+1, -1\}$ is the skew direction (+1 for right-skewed, −1 for left-skewed/mirrored). Under the GG parameterization

$$
x_{\text{shifted}} \sim \text{Gamma}\Big(k,\; \text{rate} = \tfrac{\sqrt{k}}{\sigma_{\mathrm{gam}}}\Big),
$$

so that $\mathbb{E}[x_{\text{shifted}}]=\sqrt{k}\,\sigma_{\mathrm{gam}}$ and $\text{Var}(x_{\text{shifted}})=\sigma_{\mathrm{gam}}^2$.

Define the feasibility bound:

$$
b_i = \max_t \left( \frac{-s_i \cdot \text{res}_{t,i}}{\sqrt{k}} \right).
$$

Any $\sigma_{\mathrm{gam},i} > b_i$ is guaranteed to satisfy the support constraint.

### $\eta$ Reparameterization

Rather than estimating $\sigma_{\mathrm{gam}}$ directly, the GG model estimates an unconstrained parameter $\eta \in \mathbb{R}$ and transforms:
...
To place a $\text{Log-Normal}(0, 0.5)$ prior on $\sigma_{\mathrm{gam}}$, we apply the change-of-variables formula. The Jacobian of the transformation $\sigma_{\mathrm{gam}} = b + \exp(\eta)$ is:

$$
\frac{d\sigma_{\mathrm{gam}}}{d\eta} = \exp(\eta)
$$

The log-posterior contribution is:

$$
\log p(\sigma_{\mathrm{gam}}) + \log\left|\frac{d\sigma_{\mathrm{gam}}}{d\eta}\right| = \text{lognormal\_lpdf}(\sigma_{\mathrm{gam}} \mid 0, 0.5) + \eta
$$
...
```stan
model {
  // compute feasibility bounds: b_i = max_t(-s_i * res_{t,i}) / sqrt(k)
  for (i in 1:2) {
...
## 6.5. CDF Mirroring for Left-Skewed Gamma Margins

For left-skewed (mirrored) Gamma margins, the model must correctly transform the CDF to preserve the copula correlation structure.

### The Mirroring Problem

Let $X \sim \text{Gamma}(k, \beta)$ with CDF $F_X(x)$. For a mirrored variable $Y = -X$:
...
The GG model applies the CDF flip after computing the Gamma CDF:

```stan
for (i in 1:2) {
  // shift residual to positive support
  x_shifted[i] = sqrt_shape * sigma_gam[i] + skew_direction[i] * res[i];
