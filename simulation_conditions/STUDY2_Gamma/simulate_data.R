...
generate_residuals <- function(n, m1, m2, copula_rho, eps = 1e-9) {
  ...
  qfun <- function(m) {
    switch(m$type,
      ...
      # STUDY 3 ADDITION: Standardized Gamma
      # Convention: Gamma(shape, rate). Mean = shape/rate, SD = sqrt(shape)/rate.
      # Standardization uses: Z = (X - mean) / sd, then optional mirroring for left skew.
      gamma = {
        shape <- m$shape
        rate <- m$rate

        if (is.null(shape) || is.null(rate)) {
          stop("gamma margins require 'shape' and 'rate' in margin_info")
        }
        if (!is.finite(shape) || !is.finite(rate) || shape <= 0 || rate <= 0) {
          stop("gamma margins require shape>0 and rate>0")
        }
