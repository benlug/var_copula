...
# Paths (consistent with Studies 1–3)
# - If you render from within `simulation_conditions/STUDY4/`, these resolve to `./data` and `./results`.
# - If you render from the project root, these fall back to `simulation_conditions/STUDY4/`.
...
if (!is.null(design)) {
  design_aug <- design |>
    ...
    select(
      condition_id, N_units, T, burnin, rho, VARset, direction,
...
if (is.null(design)) {
  cat(sprintf("(Design file not found: `%s`. Skipping design tables.)", files$design))
} else {
  design_tbl <- design |>
    mutate(
      tau_mu_1 = vapply(tau_mu, function(v) v[1], numeric(1)),
...
This report reads the summary artifacts produced by `run_pipeline_ml.R` and `analysis_multilevel.R` (paths shown relative to the Study 4 directory):

- `data/sim_conditions_ml.rds` (optional; simulation design table used for reporting).
- `results/summary_replications_ml.csv` (required; per-replication parameter summaries and diagnostics).
- `results/summary_conditions_ml.csv` (required; condition-level summaries: mean bias, coverage, etc.).
