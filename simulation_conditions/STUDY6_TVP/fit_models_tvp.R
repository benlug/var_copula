#!/usr/bin/env Rscript
###########################################################################
# fit_models_tvp.R  – Study 6: Time-Varying Parameter Copula-VAR
###########################################################################
#
# Fits four model variants:
#   - TVP_NG:   Time-varying rho, Normal margins
#   - TVP_EG:   Time-varying rho, Exponential margins
#   - Const_NG: Constant rho, Normal margins (baseline)
#   - Const_EG: Constant rho, Exponential margins (baseline)
#
###########################################################################

fit_tvp_copula_models <- function(data_dir,
                                  fits_dir,
                                  stan_dir,
                                  results_dir,
                                  chains = 4,
                                  ...
                                  start_condition = 1,
                                  start_rep = 1,
                                  models_to_fit = c("TVP_NG", "Const_NG")) {
  ...
  ## -- load models -----------------------------------------------------------
  all_models <- list(
    TVP_NG   = model_cache(file.path(stan_dir, "model_TVP_NG_sl.stan"), "TVP_NG"),
    TVP_EG   = model_cache(file.path(stan_dir, "model_TVP_EG_sl.stan"), "TVP_EG"),
    Const_NG = model_cache(file.path(stan_dir, "model_Const_NG_sl.stan"), "Const_NG"),
    Const_EG = model_cache(file.path(stan_dir, "model_Const_EG_sl.stan"), "Const_EG")
  )
  
  # Filter to requested models
  models <- all_models[intersect(names(all_models), models_to_fit)]
  ...
  make_init_tvp_ng <- function(T_len) {
    list(
      mu = rnorm(2, 0, 0.1),
      phi11 = runif(1, -0.5, 0.5), phi12 = runif(1, -0.3, 0.3),
      phi21 = runif(1, -0.3, 0.3), phi22 = runif(1, -0.5, 0.5),
      sigma = runif(2, 0.8, 1.2),
      z0 = rnorm(1, 0, 0.3),
      sigma_z = runif(1, 0.01, 0.05),
      z_innov = rnorm(T_len - 2, 0, 1)
    )
  }
  
  make_init_const_eg <- function(y_data, skew_dir) {
  ...
  make_init_tvp_eg <- function(y_data, skew_dir, T_len) {
    base <- make_init_const_eg(y_data, skew_dir)
    base$rho <- NULL  # Remove scalar rho
    base$z0 <- rnorm(1, 0, 0.3)
    base$sigma_z <- runif(1, 0.01, 0.05)
    base$z_innov <- rnorm(T_len - 2, 0, 1)
    base
  }
  ...
  log_vec <- foreach::foreach(
    ...
    T_len <- ds$T
    sdat_base <- list(T = T_len, y = y_matrix)
    ...
    # Determine which models to run based on margin type
    margin_type <- ds$true_params$margin1$type %||% "normal"
    is_exp <- identical(margin_type, "exponential")
    
    # Filter models based on data type
    models_for_data <- if (is_exp) {
      intersect(names(models), c("TVP_EG", "Const_EG", "TVP_NG", "Const_NG"))
    } else {
      intersect(names(models), c("TVP_NG", "Const_NG"))
    }

    for (code in models_for_data) {
      ...
      if (code == "TVP_NG") {
        inits <- replicate(chains, make_init_tvp_ng(T_len), simplify = FALSE)
      } else if (code == "Const_NG") {
        inits <- replicate(chains, make_init_const_ng(), simplify = FALSE)
      } else if (code == "TVP_EG") {
        sdat$skew_direction <- skew_directions
        inits <- lapply(1:chains, function(k) make_init_tvp_eg(y_matrix, skew_directions, T_len))
      } else if (code == "Const_EG") {
        sdat$skew_direction <- skew_directions
        inits <- lapply(1:chains, function(k) make_init_const_eg(y_matrix, skew_directions))
      }
      ...
      fit <- withCallingHandlers(
        tryCatch(
          rstan::sampling(
            ...
            thin = 2,                    # Thin samples to reduce memory
            seed = seeds_stan[1],
            init = inits,
