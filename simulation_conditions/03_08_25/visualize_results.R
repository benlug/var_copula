...
if (dir.exists(file.path(BASE, "results_S2"))) {
  STUDY_TAG <- "S2"
  message("Visualizing Study 2 results.")
  DIR <- list(
    data    = file.path(BASE, "data_S2"),
    res     = file.path(BASE, "results_S2"),
    plots   = file.path(BASE, "results_S2", "plots"),
    plots_g = file.path(BASE, "results_S2", "plots_global")
  )
  DESIGN_FILE <- "sim_conditions_S2.rds"
} else {
  STUDY_TAG <- "S1"
